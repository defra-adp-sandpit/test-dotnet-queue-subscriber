...
test-dotnet-queue-subscriber Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-dotnet-queue-subscriber-local` in docker compose files    |
...
`docker-compose -p test-dotnet-queue-subscriber-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-dotnet-queue-subscriber-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-dotnet-queue-subscriber-snd1
    - test-dotnet-queue-subscriber-snd2
    - test-dotnet-queue-subscriber-snd3
variables:
    - test-dotnet-queue-subscriber-APPLICATION-SECRET
```
